document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Removed theme toggle logic as requested
  ...
  try {
    ...
    document.addEventListener("click", (event) => {
      ...
      modalRefs.apiResponseBody.innerHTML = ""; // Use innerHTML for media
      ...
      modalRefs.submitBtn.onclick = async () => {
        ...
        if (!isValid) {
          modalRefs.apiResponseCode.textContent = "400 Bad Request";
    ...
    document.getElementById("downloadResponse").addEventListener("click", () => {
      ...
      const filename = "response.json"; // Default to JSON
      let mimeType = "application/json";

      // Check if the content is an image or video URL
      const imgElement = document.getElementById("apiResponseBody").querySelector("img");
      const videoElement = document.getElementById("apiResponseBody").querySelector("video");

      if (imgElement) {
        const imgUrl = imgElement.src;
        const imgExt = imgUrl.split('.').pop().split('?')[0];
        filename = `response.${imgExt}`;
      ...
      const blob = new Blob([responseText], { type: mimeType });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = filename;
    ...
    async function handleApiRequest(apiUrl, modalRefs, apiName) {
      ...
      modalRefs.apiResponseBody.innerHTML = "Loading..."; // Use innerHTML for media
      ...
      try {
        ...
        const statusText = statusCodeDescriptions[response.status] || "Unknown Status";
        modalRefs.apiResponseCode.textContent = `${response.status} ${statusText}`;
        modalRefs.apiResponseHeaders.textContent = Object.entries(headers).map(([key, value]) => `${key}: ${value}`).join('\n');

        const contentType = response.headers.get("Content-Type");
        modalRefs.apiResponseBody.innerHTML = ""; // Clear loading text
        ...
        if (contentType && contentType.startsWith("image/")) {
          ...
          video.src = videoUrl;
          video.controls = true;
          video.autoplay = false; // Autoplay can be annoying, set to false by default
          video.loop = false;
          video.muted = false;
          video.style.maxWidth = "100%";
          video.style.height = "auto";
          video.style.borderRadius = "8px";
          modalRefs.apiResponseBody.appendChild(video);
        } else {
          // Assume JSON or plain text
          try {
            ...
            modalRefs.apiResponseBody.textContent = textData;
          }
        }
      } catch (error) {
        modalRefs.apiResponseCode.textContent = `500 Internal Server Error`;
