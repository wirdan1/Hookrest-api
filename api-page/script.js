  ...
  try {
    ...
    settings.categories.forEach((category) => {
      ...
      <div class="${itemClass}" data-name="${item.name}" data-desc="${item.desc}">
        <div class="hero-section">
          ...
          <button class="btn get-api-btn" data-api-path="${item.path}" data-api-name="${item.name}" data-api-desc="${item.desc}">
            GET
          </button>
        </div>
      </div>
    `
      ...
      apiContent.insertAdjacentHTML(
        "beforeend",
        `<h3 class="mb-3 category-header" style="font-size: 22px;">${category.name}</h3><div class="row">${categoryContent}</div>`,
      )
    })
    ...
    async function handleApiRequest(apiUrl, modalRefs, apiName) {
      ...
      try {
        ...
        modalRefs.spinner.classList.add("d-none")
        modalRefs.content.classList.remove("d-none")
        document.getElementById("copyResponse").classList.remove("d-none")
      }
    }
