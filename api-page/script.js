...
try {
  ...
  settings.categories.forEach((category) => {
    ...
    categoryHeader.className = "api-category-header collapsed";
    ...
    sortedItems.forEach((item) => {
      ...
      detailContent.innerHTML = `
        ...
        <div class="tabs">
          <button class="tab-button active" data-tab="parameters-${item.name.replace(/\s/g, '-')}" data-api-path="${item.path}">Parameters</button>
          <button class="tab-button" data-tab="try-it-out-${item.name.replace(/\s/g, '-')}" data-api-path="${item.path}">Try it out</button>
        </div>

        <div class="tab-content-pane active" id="parameters-tab-${item.name.replace(/\s/g, '-')}">
        ...
        <div class="tab-content-pane" id="try-it-out-tab-${item.name.replace(/\s/g, '-')}" style="display:none;">
          <div class="response-section">
            <div class="response-header">
              <span>Server response</span>
            </div>
            <div class="tabs">
              <button class="tab-button active" data-tab="response-body-${item.name.replace(/\s/g, '-')}" data-api-path="${item.path}">Response body</button>
              <button class="tab-button" data-tab="response-headers-${item.name.replace(/\s/g, '-')}" data-api-path="${item.path}">Response headers</button>
            </div>
            <div class="tab-content-pane active" id="response-body-tab-${item.name.replace(/\s/g, '-')}" style="display:block;">
            ...
            <div class="tab-content-pane" id="response-headers-tab-${item.name.replace(/\s/g, '-')}" style="display:none;">
  ...
  document.querySelectorAll('.tab-button').forEach(button => {
    button.addEventListener('click', (event) => {
      ...
      const parentContent = parentTabs.nextElementSibling; // Assuming content is right after tabs

      // Deactivate all tabs and hide all content panes in this group
      parentTabs.querySelectorAll('.tab-button').forEach(btn => btn.classList.remove('active'));
      parentContent.querySelectorAll('.tab-content-pane').forEach(pane => pane.style.display = 'none');
  ...
  function generateParameterInputs(apiPath, container) {
    ...
    const url = new URL(`http://example.com${apiPath}`); // Use a dummy base URL
    ...
    if (paramNames.length === 0) {
      ...
      container.closest('.tab-content-pane').querySelector('.action-buttons').style.display = 'none';
      return;
    }

    container.closest('.tab-content-pane').querySelector('.action-buttons').style.display = 'flex';
  ...
  document.querySelectorAll('.execute-api-btn').forEach(button => {
    button.addEventListener('click', async (event) => {
      ...
      try {
        ...
        detailContent.querySelector(`.tab-button[data-tab^="try-it-out-"]`).click();
        detailContent.querySelector(`.tab-button[data-tab^="response-body-"]`).click(); // Ensure response body is active
      }
    });
  });
  ...
  document.querySelectorAll('.clear-inputs-btn').forEach(button => {
    button.addEventListener('click', (event) => {
      ...
      detailContent.querySelector('.api-response-content').textContent = '';
    ...
    apiCategories.forEach(categoryContainer => {
      ...
      categoryCollapseContent.querySelectorAll('.api-endpoint-card').forEach(item => {
        ...
        if (isVisible) {
          ...
          const detailId = item.querySelector('.toggle-details-btn').dataset.apiItemId;
