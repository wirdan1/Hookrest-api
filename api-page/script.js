document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    document.addEventListener("click", (event) => {
      ...
      const hasParams = params && params.toString().length > 0;
      ...
      if (hasParams) {
        ...
        Array.from(params.keys()).forEach((param) => {
          ...
          paramDesc.textContent = `The query to ask ${param.charAt(0).toUpperCase() + param.slice(1)}`;

          const paramExample = document.createElement("p");
          paramExample.className = "param-example";
          paramExample.textContent = `Example: ${param === 'text' ? 'Hello world' : 'Paris'}`;
        ...
        updateCurlAndRequestUrl(baseApiUrl, {}, apiPath.split('?')[0]);
      }
    ...
    async function handleApiRequest(apiUrl, modalRefs, apiName) {
      ...
      try {
        ...
        // Display headers
        modalRefs.apiResponseHeaders.textContent = Object.entries(headers)
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          modalRefs.apiResponseBody.textContent = text;
        ...
        modalRefs.apiResponseCode.textContent = "Error";
        modalRefs.apiResponseBody.textContent = JSON.stringify({
          error: error.message,
          details: "Failed to fetch the API endpoint"
        }, null, 2);
        modalRefs.apiResponseHeaders.textContent = "Content-Type: application/json";
      }
    }
